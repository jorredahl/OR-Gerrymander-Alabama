...
packages <- c("tidyverse", "here", "sf", "tmap", "tidycensus", "lwgeom", "knitr")
...
Each of the three compactness scores ranks the scores of each district differently. As a result, it's unclear which score most aptly fits to the cacluated `concentration_val` statistic for representation. Here, each district is mapped by their concentration value and compactness score along each metric.

```{r}
districts23_results_plot <- districts23 |> 
  st_drop_geometry() |> 
  select(DISTRICT, pct_black, concentration_val, ppir_compact, ch_compact, mbc_compact) |> 
  pivot_longer(cols = ends_with("compact"))

districts23_results_plot |> ggplot() +
  aes(x = concentration_val, y = value) +
